...
\footnote{Consider the example of a battery constructed of a Zn cathode and Cu anode in a Cu\subs2O/Zn\subs2O electrolyte connected via a salt bridge. The half-reactions are \cee{Cu^{2+} + 2e- -> Cu} ($\Esp=\SI{0.339}{V}$) and \cee{Zn -> Zn^{2+} + 2e-} ($\Esp=\SI{-0.76}{V}$) with an overall reaction \cee{Zn + Cu^{2+} -> Cu + Zn^{2+}}. The potential of this reaction is $\phi_{Cu}-\phi_{Zn}=0.339--0.76=\SI{1.099}{V}$.}
