...
\footnote{Consider a battery constructed of a Zn cathode and Cu anode in a Cu\subs2O/Zn\subs2O electrolyte and connected via a salt bridge. The half-reactions are \ce{Cu^{2+} + 2e- -> Cu} ($\Esp=\SI{0.339}{V}$) and \ce{Zn -> Zn^{2+} + 2e-} ($\Esp=\SI{-0.76}{V}$) with an overall reaction \ce{Zn + Cu^{2+} -> Cu + Zn^{2+}}. The potential of this reaction is $\phi_{\mathrm{Cu}}-\phi_{\mathrm{Zn}}=\SI{1.099}{V}$.}
...
% Potential reactions
Since the actual composition of ECF60 is proprietary it's exact redox reaction is unknown. Known information includes its sulphite-basis, consisting of \SI{0.05}{M} Au and \SI{0.24}{M} Na\subs2SO\subs3 at a pH of 9.5 \cite{roy2009electrochemical}. The expected reduction reaction for a sulphite-based Au precursor is \cite{green2007gold},
\begin{equation}
